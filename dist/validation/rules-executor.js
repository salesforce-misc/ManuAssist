 ...
 */
async function executeCheck(check, targetOrg) {
    try {
        switch (check.type) {
            case "soql":
 ...
 */
async function executeSoqlCheck(check, targetOrg, useToolingApi = false) {
    if (!check.query) {
    ...
    if (!result.success) {
        ...
        if (result.error?.includes("sObject type") &&
 ...
 */
async function executeSoqlCountCheck(check, targetOrg, useToolingApi = false) {
    if (!check.query) {
 ...
 */
async function executeCompositeCheck(check, targetOrg) {
 ...
 * - "true" (always pass)
 */
function evaluateExpectCondition(expect, context) {
    try {
 ...
 */
export async function runAudit(targetOrg, options = {}) {
    ...
    if (group) {
        ...
        if (rulesToRun.length === 0) {
            ...
            if (!groupExists) {
                throw new Error(`Validation group '${group}' not found. Available groups: ${groups.map((g) => g.id).join(", ")}`);
            }
        }
    }
 ...
 */
export function formatAuditResults(audit) {
 ...
 */
function formatFailedCheck(result) {
...
//# sourceMappingURL=rules-executor.js.map