import { z } from "zod";
import { getModuleList, getModuleContent, searchKnowledge, getHelpDocList, getHelpDocContent, getExerciseList, getExerciseContent, getGuideList, getGuideContent, getTroubleshootingList, getTroubleshootingContent, formatCitation, } from "../knowledge-loader.js";
export function register(server) {
    server.tool("list_mfg_modules", "List all available Manufacturing Cloud modules with documentation", {}, async () => {
    ...
    server.tool("get_mfg_module_docs", "Get detailed documentation for a specific Manufacturing Cloud module", {
        module: z
            .string()
            .describe("The module slug (e.g., 'account-management', 'sample-management', 'visit-management')"),
    }, async ({ module }) => {
    ...
    server.tool("search_mfg_knowledge", "Search the Manufacturing Cloud knowledge base for specific topics or terms", {
        query: z
            .string()
            .describe("The search term or phrase to look for (e.g., 'sales agreement', 'warranty claim', 'account forecast')"),
        maxResults: z
            .number()
            .optional()
    ...
    server.tool("explain_mfg_concept", "Get an explanation of a Manufacturing Cloud concept, feature, or component", {
        concept: z
            .string()
            .describe("The concept to explain (e.g., 'Sales Agreement', 'Account Forecast', 'DPE', 'Warranty Claim', 'Partner Visit')"),
    }, async ({ concept }) => {
    ...
    server.tool("get_mfg_admin_setup", "Get admin setup and configuration guidance for a Manufacturing Cloud feature", {
        feature: z
            .string()
            .describe("The feature to get admin setup for (e.g., 'sales-agreements', 'warranty-management', 'partner-visits', 'advanced-account-forecasting')"),
    }, async ({ feature }) => {
    ...
    server.tool("list_mfg_help_docs", "List all official Salesforce help documentation for Manufacturing Cloud", {}, async () => {
    ...
    server.tool("get_mfg_help_doc", "Get official Salesforce help documentation for a specific Manufacturing Cloud topic", {
        slug: z
            .string()
            .describe("The document slug (e.g., 'account-management', 'sample-management', 'activity-plans')"),
    }, async ({ slug }) => {
    ...
    // Tool: Get Guide
    server.tool("get_mfg_guide", "Get Manufacturing Cloud guide documentation (developer guide, admin guide, etc.)", {
        slug: z
            .string()
    ...
    server.tool("get_mfg_troubleshooting", "Get Manufacturing Cloud troubleshooting documentation including common issues and their resolutions", {
        topic: z
            .string()
            .optional()
    ...
    server.tool("get_mfg_exercise", "Get hands-on exercise documentation for Manufacturing Cloud training", {
        slug: z
            .string()
            .describe("The exercise slug (e.g., 'manufacturing-cloud-partner-enablement-hands-on-exercises-day-1-foundations')"),
    }, async ({ slug }) => {
