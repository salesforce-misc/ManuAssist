...
export function register(server) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    server.tool("check_sales_agreement_config", `Check Manufacturing Cloud Sales Agreement configuration. Queries SalesAgreement records, products, schedules, account associations, status distribution, and activation settings.
...
WRONG names (DO NOT USE): SalesAgreement__c, SalesContract__c, RunRateBusiness__c.`, {
        ...
        try {
            ...
            if (prodResult.success && prodResult.data?.records?.[0]) {
                const total = prodResult.data.records[0].total;
            ...
            if (scheduleResult.success && scheduleResult.data?.records?.[0]) {
                const total = scheduleResult.data.records[0].total;
    ...
    // ─────────────────────────────────────────────────────────────────────────
    server.tool("get_sales_agreement_details", "Get full details of a Sales Agreement including products and schedules. Provide a Sales Agreement name or Id.", {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    server.tool("activate_sales_agreement", "Activate a Sales Agreement by updating its Status to 'Active'. Requires the Sales Agreement Id.", {
...
//# sourceMappingURL=sales-agreements.js.map