...
export function register(server) {
    server.tool("list_users", "List active Manufacturing Cloud users with optional filters. Filter by profile, permission set, territory, or find users without territories or permission sets. CORRECT object names: User, PermissionSetAssignment, UserTerritory2Association.", {
    ...
    server.tool("list_permission_sets", "List Manufacturing Cloud permission sets and permission set licenses (PSLs) with assignment counts and capacity. Shows which permission sets exist, how many users are assigned, and PSL capacity (total vs used vs available).", {
        ...
        try {
            ...
            if (psResult.success && psResult.data?.records) {
                ...
                if (records.length === 0) {
                    ...
                    const psIds = records.map((r) => `'${r.Id}'`).join(",");
    ...
    server.tool("assign_permission_set", "Assign a permission set to one or more users. Looks up the permission set by label or API name, resolves usernames to IDs if needed, skips users who already have the assignment, and creates PermissionSetAssignment records.", {
        ...
        try {
            ...
            if (existingResult.success && existingResult.data?.records) {
                for (const rec of existingResult.data.records) {
                    alreadyAssigned.add(String(rec.AssigneeId));
                }
            }
    ...
    server.tool("unassign_permission_set", "Remove a permission set assignment from one or more users. Finds existing PermissionSetAssignment records and deletes them.", {
        permissionSetName: z.string().describe("Permission set label (e.g., 'ManufacturingSalesUser') or API name"),
        ...
        try {
            ...
            if (usernames?.length) {
                ...
                if (userResult.success && userResult.data?.records) {
                    for (const rec of userResult.data.records) {
                        resolvedIds.push(String(rec.Id));
                    }
                }
            }
            if (resolvedIds.length === 0) {
