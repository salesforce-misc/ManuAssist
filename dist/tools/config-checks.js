...
export function register(server) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    server.tool("check_mfg_user_config", `Check Manufacturing Cloud user configuration. Validates package licenses, permission set assignments, profiles, roles, and territory-user associations.
Key permission sets: ManufacturingSalesUser, ManufacturingServiceUser, ManufacturingPartnerCommunityUser, ManufacturingAnalyticsUser, WarrantyManagementUser, SalesAgreementsUser, RebateManagementUser.`, {
        ...
        try {
            ...
            if (userResult.success && userResult.data?.records?.[0]) {
                const total = userResult.data.records[0].total;
            ...
            if (noMfgResult.success && noMfgResult.data?.records?.[0]) {
                const total = noMfgResult.data.records[0].total;
            ...
            if (roleResult.success && roleResult.data?.records?.length) {
                ...
                message += roles.map((r) => `- ${r.Name}`).join("\n") + "\n\n";
            }
            else {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    server.tool("check_product_portfolio_config", `Check Manufacturing Cloud Product Portfolio / Product Catalog Management configuration.
...
WRONG names (DO NOT USE): Product__c, ProductCatalog__c, ProductCategory__c.`, {
        ...
        try {
            ...
            if (pbResult.success && pbResult.data?.records?.length) {
                ...
                message += "\n";
                const activeStandard = pbResult.data.records.find((r) => r.IsStandard === true && r.IsActive === true);
                if (!activeStandard) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    server.tool("check_mfg_account_config", "Check Account configuration for Manufacturing Cloud — account record types, relationship center setup, and partner/distributor account presence.", {
        targetOrg: z.string().optional(),
    }, async ({ targetOrg }) => {
        const validation = await validateOrgConnection();
        const effectiveOrg = targetOrg || validation.targetOrg;
        ...
        try {
            ...
            if (rtResult.success && rtResult.data?.records?.length) {
                ...
                const active = recs.filter((r) => r.IsActive === true);
            ...
            if (acctResult.success && acctResult.data?.records?.[0]) {
                const total = acctResult.data.records[0].total;
...
//# sourceMappingURL=config-checks.js.map