...
export function register(server) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    server.tool("check_partner_visit_config", `Check Manufacturing Cloud Partner Visit Management configuration. Queries Visit record types, PartnerVisit records, action plan templates, visit tasks, and permission assignments.
...
WRONG names (DO NOT USE): PartnerVisit__c, Visit__c, VisitTask__c, VisitActionPlan__c.`, {
        ...
        try {
            ...
            if (aptResult.success && aptResult.data?.records?.length) {
                ...
                const activeTemplates = templates.filter((r) => r.IsActive === true);
            ...
            if (apResult.success && apResult.data?.records?.[0]) {
                const total = apResult.data.records[0].total;
    ...
    // list_partner_visits
    // ─────────────────────────────────────────────────────────────────────────
    server.tool("list_partner_visits", "List recent partner visits with status, account, and owner. Filter by status or account name.", {
        ...
        try {
            ...
            for (const r of result.data.records) {
                ...
                const start = rec.PlannedVisitStartTime
                    ? new Date(rec.PlannedVisitStartTime).toLocaleDateString()
                    : "—";
...
//# sourceMappingURL=partner-visits.js.map