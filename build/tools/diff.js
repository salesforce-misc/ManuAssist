...
export function register(server) {
    server.tool("diff_orgs", "Compare Manufacturing Cloud configuration between two Salesforce orgs. Compares permission set assignments, Sales Agreement record types, Warranty Terms, product catalog, and key record counts to identify configuration drift. Both orgs must be authenticated via SF CLI.", {
        ...
        const fmtVal = (v) => v !== null && v !== undefined && String(v) !== "" ? String(v) : "(empty)";
        const sections = [];
        const summary = [];
        ...
        if (effectiveScope === "all" || effectiveScope === "permissions") {
            ...
            if (srcPs.success && tgtPs.success) {
                ...
                for (const r of (srcPs.data?.records || [])) {
                    const rec = r;
                    srcMap.set(rec["PermissionSet.Name"], Number(rec.cnt));
                }
                for (const r of (tgtPs.data?.records || [])) {
                    const rec = r;
                    tgtMap.set(rec["PermissionSet.Name"], Number(rec.cnt));
                }
        ...
        if (effectiveScope === "all" || effectiveScope === "sales-agreements") {
            ...
            if (srcRt.success && tgtRt.success) {
                const srcMap = new Map();
                const tgtMap = new Map();
                for (const r of (srcRt.data?.records || [])) {
                    const rec = r;
                    srcMap.set(rec.DeveloperName, rec);
                }
                for (const r of (tgtRt.data?.records || [])) {
                    const rec = r;
                    tgtMap.set(rec.DeveloperName, rec);
                }
                ...
                for (const key of [...allKeys].sort()) {
                    ...
                    if (src && tgt) {
                        if (src.IsActive === tgt.IsActive) {
                            matches++;
                            section += `| ${key} | ${fmtVal(src.IsActive)} | ${fmtVal(tgt.IsActive)} | ✅ Match |\n`;
                        }
                        else {
                            differences++;
                            section += `| ${key} | ${fmtVal(src.IsActive)} | ${fmtVal(tgt.IsActive)} | ⚠️ Different |\n`;
                        }
                    }
                    else if (src && !tgt) {
                        onlySource++;
                        section += `| ${key} | ${fmtVal(src.IsActive)} | — | ⚠️ Source only |\n`;
                    }
                    else {
                        onlyTarget++;
                        section += `| ${key} | — | ${fmtVal(tgt.IsActive)} | ⚠️ Target only |\n`;
                    }
                }
                if (allKeys.size === 0)
                    section += "_No Sales Agreement record types found in either org._\n";
            }
            else {
        ...
        if (effectiveScope === "all" || effectiveScope === "warranty") {
            ...
            if (srcWt.success && tgtWt.success) {
                const srcMap = new Map();
                const tgtMap = new Map();
                for (const r of (srcWt.data?.records || [])) {
                    const rec = r;
                    srcMap.set(rec.Name, rec);
                }
                for (const r of (tgtWt.data?.records || [])) {
                    const rec = r;
                    tgtMap.set(rec.Name, rec);
                }
                ...
                for (const key of [...allKeys].sort()) {
                    ...
                    if (src && tgt) {
                        ...
                        if (match) {
                            matches++;
                            section += `| ${key} | Active:${src.IsActive}, ${src.WarrantyDuration}${src.WarrantyUnit} | Active:${tgt.IsActive}, ${tgt.WarrantyDuration}${tgt.WarrantyUnit} | ✅ Match |\n`;
                        }
                        else {
                            differences++;
                            section += `| ${key} | Active:${src.IsActive}, ${src.WarrantyDuration}${src.WarrantyUnit} | Active:${tgt.IsActive}, ${tgt.WarrantyDuration}${tgt.WarrantyUnit} | ⚠️ Different |\n`;
                        }
                    }
                    else if (src && !tgt) {
                        onlySource++;
                        section += `| ${key} | Active:${src.IsActive} | — | ⚠️ Source only |\n`;
                    }
                    else {
                        onlyTarget++;
                        section += `| ${key} | — | Active:${tgt.IsActive} | ⚠️ Target only |\n`;
                    }
                }
                if (allKeys.size === 0)
                    section += "_No Warranty Terms found in either org._\n";
            }
            else {
        ...
        if (effectiveScope === "all" || effectiveScope === "products") {
            ...
            const srcCount = srcProd.success ? Number(srcProd.data?.records?.[0]?.total ?? 0) : "Error";
            const tgtCount = tgtProd.success ? Number(tgtProd.data?.records?.[0]?.total ?? 0) : "Error";
            section += `| Metric | ${sourceOrg} | ${targetOrg} |\n`;
            section += `|--------|-------------|-------------|\n`;
            section += `| Active Products | ${srcCount} | ${tgtCount} |\n`;
            const srcPbs = srcPb.success ? (srcPb.data?.records || []).map(r => r.Name) : [];
            const tgtPbs = tgtPb.success ? (tgtPb.data?.records || []).map(r => r.Name) : [];
