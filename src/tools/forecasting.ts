...
export function register(server: McpServer) {
  ...
  server.tool(
    "check_forecasting_config",
    `Check Manufacturing Cloud Advanced Account Forecasting configuration. Validates AccountForecast records, forecast periods, Data Processing Engine definitions, Account Manager Targets, and Program-Based Business setup.
...
WRONG names (DO NOT USE): Forecast__c, AccountForecast__c, ManagerTarget__c, MfgForecast__c.`,
    ...
    async ({ targetOrg }) => {
      ...
      try {
        ...
        if (afResult.success && afResult.data?.records?.[0]) {
          const total = (afResult.data.records[0] as Record<string, unknown>).total;
        ...
        if (fpmResult.success && fpmResult.data?.records?.[0]) {
          const total = (fpmResult.data.records[0] as Record<string, unknown>).total;
        ...
        if (dstrResult.success && dstrResult.data?.records?.[0]) {
          const total = (dstrResult.data.records[0] as Record<string, unknown>).total;
  ...
  server.tool(
    "check_account_manager_targets",
    "Get Account Manager Targets for a specific user or all users. Shows targets, status, and distribution details.",
    {
    ...
    async ({ ownerNameOrId, targetPeriod, targetOrg }) => {
      ...
      if (targetPeriod) whereClauses.push(`Name LIKE '%${targetPeriod}%'`);
