...
export function register(server: McpServer) {
  server.tool(
    "health_check",
    "Run a comprehensive Manufacturing Cloud org health check. Validates permission set assignments, Sales Agreement setup, Warranty configuration, Account Manager Targets, Data Processing Engine, product catalog, and general Salesforce setup. Returns a summary with issues and recommendations.",
    {
    ...
    async ({ targetOrg }) => {
      ...
      if (saResult.success && saResult.data?.records?.[0]) {
        saCount = Number((saResult.data.records[0] as Record<string, unknown>).total);
      }
      ...
      if (wtResult.success && wtResult.data?.records?.[0]) {
        wtCount = Number((wtResult.data.records[0] as Record<string, unknown>).total);
      }
      ...
      if (amtResult.success && amtResult.data?.records?.[0]) {
        amtCount = Number((amtResult.data.records[0] as Record<string, unknown>).total);
      }
      ...
      if (prodResult.success && prodResult.data?.records?.[0]) {
        prodCount = Number((prodResult.data.records[0] as Record<string, unknown>).total);
      }
      ...
      if (acctResult.success && acctResult.data?.records?.[0]) {
        acctCount = Number((acctResult.data.records[0] as Record<string, unknown>).total);
      }
      ...
      if (afResult.success && afResult.data?.records?.[0]) {
        afCount = Number((afResult.data.records[0] as Record<string, unknown>).total);
      }
      ...
      if (pkgResult.success && pkgResult.data?.records?.length) {
        for (const r of pkgResult.data.records) {
          const rec = r as Record<string, unknown>;
          const pkg = rec.SubscriberPackage as Record<string, unknown> | null;
  ...
  server.tool(
    "get_org_status",
    "Get a quick dashboard view of the connected Manufacturing Cloud org — key record counts, feature activation, and permission set coverage.",
    {
      ...
      for (const [label, query] of Object.entries(queryMap)) {
        try {
          ...
          if (result.success && result.data?.records?.[0]) {
            const total = (result.data.records[0] as Record<string, unknown>).total ?? 0;
            message += `| ${label} | ${total} |\n`;
          } else {
            message += `| ${label} | — |\n`;
          }
