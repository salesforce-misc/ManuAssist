...
export function register(server: McpServer) {
  ...
  server.tool(
    "check_warranty_config",
    `Check Manufacturing Cloud Warranty & Claims configuration. Validates WarrantyTerm records, WarrantyClaim statuses, claim rules, supplier recovery contracts, and permission coverage.
...
WRONG names (DO NOT USE): WarrantyTerm__c, WarrantyClaim__c, WarrantyContract__c.`,
    ...
    async ({ targetOrg }) => {
      ...
      try {
        ...
        if (wtResult.success && wtResult.data?.records?.length) {
          ...
          const active = terms.filter((r) => (r as Record<string, unknown>).IsActive === true);
        ...
        if (claimStatusResult.success && claimStatusResult.data?.records?.length) {
          ...
          const pendingRec = claimStatusResult.data.records.find(
            (r) => (r as Record<string, unknown>).Status === "New"
          );
          if (pendingRec && Number((pendingRec as Record<string, unknown>).total) > 50) {
            recommendations.push(`${(pendingRec as Record<string, unknown>).total} claims are still in 'New' status — review the claims adjudication process`);
          }
        } else {
        ...
        if (assetResult.success && assetResult.data?.records?.[0]) {
          const total = (assetResult.data.records[0] as Record<string, unknown>).total;
  ...
  server.tool(
    "list_warranty_claims",
    "List warranty claims with status, account, and claim amounts. Filter by status or account.",
    {
      ...
      try {
        ...
        for (const r of result.data.records) {
          ...
          const claimDate = rec.ClaimDate
            ? new Date(rec.ClaimDate as string).toLocaleDateString()
            : "—";
